...
def file_too_large(file_path: str, limit: int, size: int | None = None) -> PayloadTooLargeError:
    measured = f"is {size} bytes, " if size is not None else ""
    details: dict[str, Any] = {"max_bytes": limit}
    if size is not None:
        details["size"] = size
    return PayloadTooLargeError(
        f"'{file_path}' {measured}over the {limit}-byte limit (GITLAB_MCP_MAX_FILE_BYTES). "
