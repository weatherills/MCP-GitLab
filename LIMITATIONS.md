...
- **`mcp-gitlab service start|stop|restart` runs the server without Docker,** as a detached
  ...
  (`systemd`, `schtasks`). `deploy/windows/` registers `mcp-gitlab serve` directly as a Windows
  Scheduled Task, rather than wrapping `service start` (Task Scheduler's own restart-on-failure
  needs to track the server process itself), as a courtesy for Windows users; that is not a real
  Windows Service (no Service Control Manager, no Session 0) and is not the intended deployment,
  which stays Linux + Docker (below).
